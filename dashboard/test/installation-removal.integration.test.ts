...
describe("syncInstallations() -- repo removed from installation preserves historical data", async (assert) => {
  ...
  raw.prepare("INSERT INTO installations (id, account_login, suspended_at) VALUES (?, ?, ?)").run(1, "acme", null)

  const p1 = raw.prepare("INSERT INTO projects (owner, repo, installation_id) VALUES (?, ?, ?)").run("acme", "api", 1).lastInsertRowid
  const p2 = raw.prepare("INSERT INTO projects (owner, repo, installation_id) VALUES (?, ?, ?)").run("acme", "web", 1).lastInsertRowid
  const p3 = raw.prepare("INSERT INTO projects (owner, repo, installation_id) VALUES (?, ?, ?)").run("acme", "mobile", 1).lastInsertRowid

  // Seed test data for the project that will be removed
  raw.prepare("INSERT INTO test_runs (project_id, run_id, branch, commit_sha, timestamp, total_tests, passed_tests, failed_tests, flaky_tests, unresolved_tests) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(p3, "run-1", "main", "abc123", "2026-04-01T00:00:00Z", 10, 9, 0, 1, 0)
  raw.prepare("INSERT INTO quarantined_tests (project_id, test_id, name, quarantined_at) VALUES (?, ?, ?, ?)").run(p3, "test-1", "flaky checkout test", "2026-04-01T00:00:00Z")
  ...
  try {
    ...
    const apiProject = raw.prepare("SELECT installation_id FROM projects WHERE owner = ? AND repo = ?").get("acme", "api") as { installation_id: number | null }
    ...
    const webProject = raw.prepare("SELECT installation_id FROM projects WHERE owner = ? AND repo = ?").get("acme", "web") as { installation_id: number | null }
    ...
    const mobileProject = raw.prepare("SELECT installation_id FROM projects WHERE owner = ? AND repo = ?").get("acme", "mobile") as { installation_id: number | null }
    ...
    const testRunCount = raw.prepare("SELECT COUNT(*) as count FROM test_runs WHERE project_id = ?").get(p3) as { count: number }
    ...
    const qtCount = raw.prepare("SELECT COUNT(*) as count FROM quarantined_tests WHERE project_id = ?").get(p3) as { count: number }
...
describe("syncInstallations() -- uninstalled app detected and marked as removed", async (assert) => {
  ...
  raw.prepare("INSERT INTO installations (id, account_login, suspended_at) VALUES (?, ?, ?)").run(1, "org1", null)
  raw.prepare("INSERT INTO installations (id, account_login, suspended_at) VALUES (?, ?, ?)").run(2, "org2", null)

  raw.prepare("INSERT INTO projects (owner, repo, installation_id) VALUES (?, ?, ?)").run("org1", "repo1", 1)
  const p2 = raw.prepare("INSERT INTO projects (owner, repo, installation_id) VALUES (?, ?, ?)").run("org2", "repo2", 2).lastInsertRowid

  // Seed test data for org2/repo2
  raw.prepare("INSERT INTO test_runs (project_id, run_id, branch, commit_sha, timestamp, total_tests, passed_tests, failed_tests, flaky_tests, unresolved_tests) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(p2, "run-2", "main", "def456", "2026-04-02T00:00:00Z", 20, 18, 1, 1, 0)
  raw.prepare("INSERT INTO quarantined_tests (project_id, test_id, name, quarantined_at) VALUES (?, ?, ?, ?)").run(p2, "test-2", "flaky payment test", "2026-04-02T00:00:00Z")
  ...
  const repoPayload = {
    total_count: 1,
    repositories: [
      { owner: { login: "org1" }, name: "repo1", full_name: "org1/repo1" },
    ],
  }
  ...
  try {
    ...
    const inst1 = raw.prepare("SELECT removed_at FROM installations WHERE id = ?").get(1) as { removed_at: string | null }
    ...
    const inst2 = raw.prepare("SELECT removed_at FROM installations WHERE id = ?").get(2) as { removed_at: string | null }
    ...
    const repo1 = raw.prepare("SELECT installation_id FROM projects WHERE owner = ? AND repo = ?").get("org1", "repo1") as { installation_id: number | null }
    ...
    const repo2 = raw.prepare("SELECT installation_id FROM projects WHERE owner = ? AND repo = ?").get("org2", "repo2") as { installation_id: number | null }
    ...
    const testRunCount = raw.prepare("SELECT COUNT(*) as count FROM test_runs WHERE project_id = ?").get(p2) as { count: number }
    ...
    const qtCount = raw.prepare("SELECT COUNT(*) as count FROM quarantined_tests WHERE project_id = ?").get(p2) as { count: number }
