...
1. **CLI-native App auth:** Build JWT generation (RS256), `TokenProvider` interface, `InstallationTokenProvider` with caching/refresh directly in the Go CLI. Requires 3 new env vars (`QUARANTINE_APP_CLIENT_ID`, `QUARANTINE_PRIVATE_KEY[_PATH]`, `QUARANTINE_INSTALLATION_ID`).
